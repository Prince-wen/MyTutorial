...
/*
    ...
     子类重写父类的抽象属性，需要将属性补充完整。
     子类重写父类的完整属性，那么需要增加override关键字修饰
     子类重写父类的属性时，这个属性不能时可变的（var），需要采用val声明
    abstract class Person {
      var name:String
    }
...
/*
    ...
    */
    ...
    如果一个类符合某一个特征（特质），那么就可以将这个特征（特质）“混入”到类中。
    ...
    4.将特质混入类中，采用extends关键字或with关键字
      ...
      还可以使用with混入其他的特质
      如果特质中存在抽象方法，那么混入的类应该补全方法。否则为抽象。
      如果特质中存在具体方法，子类重写时，需要override关键字。
    5.Scala可以直接使用Java代码，Scala中没有接口
      那么Java中的接口在Scala中怎么用
      Java中的所有接口在Scala中都当成特质来用
